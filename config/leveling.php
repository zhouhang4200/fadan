...
return [
	...
	'mayidailian' => [
		...
		'Ver'       => '1.0',
		'url'       => env('MAYI_URL') ?? 'http://www.mayidailian.com/OpenApi/GateWay/index', // 蚂蚁代练的接口地址
		'password'  => '123456',
	...
	'dd373' => [
		...
		'aes_iv'        => '1234567891111152',
		'url' => [
			'onSale'                   => env('DD373_URL').'?action=UpOrder', // 上架
			'offSale'                  => env('DD373_URL').'?action=DownOrder', // 下架
			'applyRevoke'              => env('DD373_URL').'?action=applyCancel', // 申请撤销
			'cancelRevoke'             => env('DD373_URL').'?action=CancelAction', // 取消撤销
			'agreeRevoke'              => env('DD373_URL').'?action=CancelAction', // 同意撤销
			'applyArbitration'         => env('DD373_URL').'?action=applyArbitrate', // 申请仲裁
			'cancelArbitration'        => env('DD373_URL').'?action=undoArbitrate', // 取消仲裁
			'complete'                 => env('DD373_URL').'?action=confirmOrder', // 订单完成
			'lock'                     => env('DD373_URL').'?action=lockAccount', // 锁定
			'cancelLock'               => env('DD373_URL').'?action=unlockAccount', // 取消锁定
			'delete'                   => env('DD373_URL').'?action=deleteOrder', // 删除订单
			'updateOrder'              => env('DD373_URL').'?action=modifyOrder', // 修改订单
			'addTime'                  => env('DD373_URL').'?action=addTime', // 加时
			'addMoney'                 => env('DD373_URL').'?action=addPrice', // 加款
			'orderDetail'              => env('DD373_URL').'?action=getDetails', // 订单详情
			'getScreenshot'            => env('DD373_URL').'?action=getImages', // 订单截图
			'getMessage'               => env('DD373_URL').'?action=getMessages', //获取留言
			'replyMessage'             => env('DD373_URL').'?action=sendMessage', // 回复留言
			'updateAccountAndPassword' => env('DD373_URL').'?action=changePwd', // 修改账号密码
			'refuseRevoke'             => env('DD373_URL').'?action=CancelAction', // 不同意撤销
			'updateImage'			   => env('DD373_URL').'?action=uploadImage', //上传截图
		],
		'status' => [
			'1'  => '未接单',
			'4'  => '代练中',
