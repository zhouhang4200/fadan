        ...
        while (orderAssignSwitchGet()) {
            ...
            foreach (waitReceivingGet() as $orderNo => $data) {
                ...
                if ($minutes >= 40 && !in_array($sendUser, [8311, 8111])) {
                    ...
                    // 如果是王者直接分配给1131
                    if (in_array($orderInfo->game_id, [3,4,1,58, 13,105])  && $orderInfo->creator_primary_user_id != 8307) {
                        try {
                        ...
                        } else if (in_array($orderInfo->game_id, [3,4,1,58, 13,105])  && $orderInfo->creator_primary_user_id != 8307) {
                            $this->assign($orderNo, 8329);
                        } else {
