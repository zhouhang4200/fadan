...
class EmployeeStatistic extends Command
{
    ...
    {   
        ...
        try {
            ...
            $userDatas = DB::select("SELECT NOW() as created_at, NOW() as updated_at, a.creator_primary_user_id as parent_id, c.name, c.user_name, DATE_FORMAT(a.updated_at, '%Y-%m-%d') AS date, a.creator_user_id as user_id, COUNT(CASE WHEN a.status = 20 THEN a.id ELSE NULL END) AS complete_order_count, SUM(CASE WHEN a.status = 20 THEN a.amount ELSE 0 END) AS send_order_amount, COUNT(CASE WHEN a.STATUS = 19 THEN a.id ELSE NULL END) AS revoke_order_count, COUNT(CASE WHEN a.STATUS = 21 THEN a.id ELSE NULL END) AS arbitrate_order_count, (SUM(CASE WHEN a.status = 20 THEN a.original_amount ELSE 0 END) - SUM(CASE WHEN a.STATUS = 20 THEN a.amount ELSE 0 END)+SUM(CASE WHEN a.status IN (19, 21) AND a.creator_user_id = b.user_id THEN b.fee ELSE 0 END)) AS profit FROM  orders a LEFT JOIN user_amount_flows b ON a.no = b.trade_no left join users c on a.creator_user_id = c.id WHERE a.updated_at >= '$yestodayDate' AND a.updated_at < '$todayDate' AND a.status IN (19, 20, 21) GROUP BY creator_user_id");
