...
class Revoked extends DailianAbstract implements DailianInterface
{
	...
	// 运行, 第一个参数为订单号，第二个参数为操作用户id, 第三个为给接单的代练费（填的协商的钱),第四个参数接口传的给发单商户返的双金, 第五个为接口传的手续费,第六个为协商的双金
    public function run($orderNo, $userId, $amount, $apiDeposit, $apiService, $writeDeposit)
    {	
    	DB::beginTransaction();
    	try {
    		// 赋值
