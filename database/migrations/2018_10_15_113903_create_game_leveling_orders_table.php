...
class CreateGameLevelingOrdersTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('game_leveling_orders', function (Blueprint $table) {
            ...
            $table->string('platform_no')->comment('外部接单平台订单号');
            ...
            $table->decimal('get_amount', 17, 4)->default(0.00)->comment('获得金额');
            ...
            $table->integer('game_type_id')->default(1)->unsigned()->comment('游戏类型ID');
            $table->integer('game_class_id')->default(1)->unsigned()->comment('游戏类别ID');
            ...
            $table->string('customer_service_name')->nullable()->comment('下单客服');
            $table->string('seller_nick')->nullable()->comment('卖家旺旺');
            $table->string('pre_sale')->nullable()->comment('接单客服');
            ...
            $table->decimal('price_increase_step', 17, 4)->default(0.00)->comment('自动加价步长');
            $table->decimal('price_ceiling', 17, 4)->default(0.00)->comment('自动加价上限');
            $table->dateTime('apply_complete_at')->nullable()->comment('申请验收时间');
            $table->dateTime('complete_at')->nullable()->comment('订单完成时间');
            $table->boolean('source')->default(1)->comment('订单来源');
            ...
            $table->string('take_user_qq', 60)->nullable()->default('0')->comment('接单用户QQ');
