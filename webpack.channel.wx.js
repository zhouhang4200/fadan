...
mix.webpackConfig({
    output: {
        publicPath: '/channel-wx/', // 设置默认打包目录
        chunkFilename: `js/[name].${mix.inProduction() ? '[chunkhash].' : ''}js` // 路由懒加载的时候打包出来的js文件
    }
});

mix.js('resources/assets/channel-wx/js/app.js', 'public/channel-wx/js') // 打包js
    ...
    .setResourceRoot('/channel-wx/') // 设置资源目录
    .setPublicPath('public/channel-wx'); // 设置 mix-manifest.json 目录