                ...
                <div class="main-box-body clearfix">
                    <div class="layui-tab layui-tab-brief" lay-filter="widgetTab">
                        <ul class="layui-tab-title">
                            <li class="layui-this" lay-id="add">添加前台账号角色</li>
                        </ul>
                        <div class="layui-tab-content">
                            <form class="layui-form" method="POST" action="{{ route('groups.store', ['userId' => $user->id]) }}">
                            {!! csrf_field() !!}
                                <div style="width: 40%">
                                                    ...
                                                    <tr>
                                                        <td>
                                                            ...
                                                            <div class="layui-input-inline">
                                                              <input type="checkbox" name="roles[]" lay-skin="primary" title="{{ $role->alias }}" {{ in_array($role->id, $userRoles) ? 'checked' : ''  }} value="{{ $role->id }}">
                                                            </div>
                                                            @endforeach
                                                            </div>
                                                        </td>
...
@section('js')
    <script>
        ...
        var storeError = "{{ session('storeError') ?: '' }}";
        var missRole = "{{ session('missRole') ?: '' }}";

        if (error) {
            layer.msg(error, {icon: 5, time:1500});        } else if(storeError) {
            layer.msg(storeError, {icon: 5, time:1500});        } else if(missRole) {
