                ...
                <div class="main-box-body clearfix">
                    <div class="layui-tab layui-tab-brief" lay-filter="widgetTab">
                        ...
                        <div class="layui-tab-content">
                            ...
                            <div class="layui-tab-item layui-show" lay-size="sm" id="region">
                                @include('backend.game.region.list')
                                {!! $regions->appends([
                                    'name' => $name
                                ])->render() !!}
                            </div>
        ...
        layui.use(['form', 'layedit', 'laydate'], function(){
            ...
            form.on('submit(delete)', function (data) {
                ...
                layer.confirm('确认删除吗？', {
                    ...
                    $.ajax({
                        type: 'post',
                        url: "{{ route('admin.region.delete') }}",
                        data:{id:id},
                        success: function (data) {
                            layer.msg(data.message);
                            if (page) {
                                $.get("{{ route('admin.region.index') }}?page="+page, function (result) {
                                    $('#region').html(result);
                                    form.render();
                                }, 'json');
                            } else {
                                $.get("{{ route('admin.region.index') }}", function (result) {
                                    $('#region').html(result);
                                    form.render();
                                }, 'json');
                            }
