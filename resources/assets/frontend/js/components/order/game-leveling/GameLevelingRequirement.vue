...
<script>
    export default {
        ...
        methods: {
            ...
            handleSubmitForm() {
                this.$api.gameLevelingRequirementTemplateStore(this.form).then(res => {
                    if (res.status == 1) {
                        this.handleFormRest();
            ...
            handleDelete(row) {
                this.$confirm('此操作将永久删除, 是否继续?', '提示', {
                    ...
                    this.$api.gameLevelingRequirementTemplateDelete({id:row.id}).then(res => {
                        if (res.status == 1) {
            ...
            handleEdit(row) {
                this.$api.gameLevelingRequirementTemplateShow({id:row.id}).then(res => {
